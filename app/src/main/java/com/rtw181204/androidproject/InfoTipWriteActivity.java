...
public class InfoTipWriteActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_info_tip_write);


        Toolbar toolbar = findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        getSupportActionBar().setTitle("팁 작성");
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.tip_write,menu);
    ...
    private void submitPost(){
        ...
        mDatabase.child("users").child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(user == null){
                    Toast.makeText(InfoTipWriteActivity.this,
                   ...
                   // FirebaseDatabase.getInstance().getReference("users").child(userId).setValue(new User(G.nickName, G.id, G.bead+2, G.date, G.attCheck));
                    //FirebaseDatabase.getInstance().getReference("users").child(userId).child("beadHistory").push().setValue(new BeadList("팁게시판", "3개", G.beadHistory));
    ...
    private void writeNewPost(String userId, String username, String title, String body) {
        ...
        String key = mDatabase.child("tip").child("posts").push().getKey();
        Post post = new Post(userId, username, title, body,date);
        Map<String, Object> postValues = post.toMap();

        Map<String, Object> childUpdates = new HashMap<>();
        childUpdates.put("/tip/posts/"+key, postValues);
        childUpdates.put("/tip/user-posts/"+userId +"/" + key, postValues);
